...
class DataController extends Controller
{
    ...
    public static function getReceiveData($request)
    {
        ...
        if ($request->page_title=='receive_list') {
            ...
            $trade_number = $request->trade_number; // 印刷
            ...
            $result=$result->leftJoin('data_shipment_vouchers as dsv', function($join){
        ...
        $receive_datas = $result->get()->toArray();

        $receive_data=array();
        $voucher_id_array=array();
        foreach ($receive_datas as $key => $value) {
            //
            $voucher_id_array[]=$value['data_receive_voucher_id'];
            unset($value['data_receive_voucher_id']);
            $receive_data[]=$value;
        }
        $voucher_id_array=array_values(array_unique($voucher_id_array));
        self::checkDateTimeChange($voucher_id_array);
        Log::debug(__METHOD__.':end---');
        return ['receive_data'=>$receive_data,'total_data'=>$result->count(),'voucher_id_array'=>$voucher_id_array];
    }
    public static function checkDateTimeChange($voucher_id_array){
        $today=date('y-m-d H:i:s');
        foreach ($voucher_id_array as $key => $voucher_id) {
            data_receive_voucher::where('data_receive_voucher_id',$voucher_id)
