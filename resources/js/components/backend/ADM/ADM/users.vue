                        ...
                        <tbody>
                            <tr v-for="(user, i) in users" :key="i">
                                ...
                                <td>
                                    ...
                                    <button type="button" class="btn btn-danger" @click.prevent="deleteOrRestoreUser(user,'d')" v-can="['user_delete']" v-if="user.deleted_at==null">
                                        <b-icon icon="trash-fill" font-scale="1.2"></b-icon> 削除</button>
                                    <button type="button" class="btn btn-info" @click.prevent="deleteOrRestoreUser(user,'r')" v-can="['user_delete']" v-else>
                                        <b-icon icon="arrow-clockwise" font-scale="1.2"></b-icon> 削除取消</button>
                                </td>
                            </tr>
                        </tbody>
                    </table>
        ...
        this.yes_btn=(type==='d'?'削除します':'削除を取り消します')
            this.confirm_sweet().then((result) => {
              if (result.value) {
                ...
                axios.delete(this.BASE_URL+'api/users/'+user.id+'?deleted_at='+user.deleted_at)
                    .then(({data})=> {
                        if (data.status==1) {
                        this.alert_icon=data.class_name;
