      ...
      <div class="col-12" style="background: #d8e3f0; padding: 10px">
        ...
        >
          ...
          <td style="width:10%" class="cl_custom_color">商品コード</td>
            ...
            <td style="width:15%">
              <select class="form-control" v-model="form.goods_classification_code">
                <option value="*">全て</option>
                <option v-for="(opt, i) in mes_lis_ord_tra_ins_goods_classification_codeList" :key="i" :value="i">
                  {{ opt }}
                </option>
              </select>
            </td>
          ...
          <div class="col-5">
            <p>
              ...
              <span class="selectPagi">
                <select @change="selectNumPerPage" v-model="form.per_page" class="form-control selectPage">
      ...
      <div class="col-12">
        <div class="">
          <table class="table table-striped table-bordered order_item_details_table" style="overflow-x: scroll">
            ...
            <tbody>
              <tr v-for="(order_detail_list, index) in receive_detail_lists.data" :key="index">
                <td>
                  {{receive_detail_lists.current_page * form.per_page - form.per_page + index + 1 }}
                </td>
                <td>
...
export default {
  ...
  methods: {
    ...
    deliverySearchForm1() {
      ...
      axios.post(this.BASE_URL + "api/slr_get_voucher_detail_popup1_receive", this.$route.query)
        .then(({ data }) => {
            // console.log(data);
    ...
    deliverySearchForm2() {
      this.order_search_modal2 = true;
       this.$route.query.adm_user_id = Globals.user_info_id;
       this.$route.query.byr_buyer_id = this.form.byr_buyer_id;
      axios.post(this.BASE_URL + "api/slr_get_voucher_detail_popup2_receive", this.$route.query)
        .then(({ data }) => {
            // console.log(data);
    ...
    deliverySearchForm3() {
      this.order_search_modal3 = true;
       this.$route.query.adm_user_id = Globals.user_info_id;
       this.$route.query.byr_buyer_id = this.form.byr_buyer_id;
      axios.post(this.BASE_URL + "api/slr_get_voucher_detail_popup3_receive", this.$route.query)
        .then(({ data }) => {
            // console.log(data);
  ...
  created() {
        this.form = this.$store.getters['slrReceiveDetailListModule/getFormData'];
