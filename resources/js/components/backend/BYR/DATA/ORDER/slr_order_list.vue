          ...
          <tbody>
            <tr v-for="(order_list, index) in order_lists.data" :key="index">
              <td>{{ order_lists.current_page *
                    ...
                    1 }}</td>
              <td>
                  <button @click="goToDetailsPage('slr_order_list_details',order_list)" class="btn btn-link btn-link-custom">
              ...
              <td>{{ order_list.deleted_at | momentDateTimeFormat }}</td>
              <span v-role="['Super Admin']">
              <td v-if="order_list.deleted_at==null">
                  <b-button pill variant="danger" @click="deleteOrRetrive(order_list,'d')">削除</b-button>
              </td>
              <td v-else><b-button pill variant="info" @click="deleteOrRetrive(order_list,'r')">削除取消</b-button></td>
              </span>
            </tr>
...
export default {
  ...
  methods: {
    ...
    showAllCustomerCode(){
     let loaders = Vue.$loading.show();
      ...
      axios.post(this.BASE_URL + "api/get_slr_customer_code_list", this.form)
        .then(({ data }) => {
    ...
    get_all_order(page=1) {
      ...
      this.form.page=page;
      axios.post(this.BASE_URL + "api/get_slr_order_list", this.form)
        .then(({ data }) => {
            // console.log(data);
    ...
    deleteOrRetrive(order_list,type='d'){
        ...
        this.confirm_sweet().then((result) => {
            if (result.value) {
                axios.post(this.BASE_URL + "api/order_delete_or_retrive", order_list).then(({ data }) => {
                    if (data.status==1) {
    ...
    goToDetailsPage(page_name,order_list){
        this.$store.commit('slrOrderDetailsModule/reset')
  ...
  created() {
      this.form = this.$store.getters['slrOrderModule/getFormData'];
