...
export default {
  ...
  data() {
    return {
        ...
        parent: { name: 'slr_order_list_details', query: {}},
        order_info:[],
        item_search_query:[],
        jan_code_modal:false,
...
//         },
  methods: {
      ...
      searchBypopupmodal(){
        this.jan_code_modal= true;
      },
    //get Table data
    getItemSearchData(page = 1) {
      let loader = Vue.$loading.show();
        this.form.page=page;
        axios.post(this.BASE_URL + "api/slr_get_shipment_item_search", this.form)
        .then(({ data }) => {
  ...
  created() {
      this.form = this.$store.getters['slrItemSearchModule/getFormData'];
