        ...
        >
          ...
          <tbody>
            <tr>
              <td>1</td>
              <td>
                  <button @click="goToItemDetailsPage('slr_payment_item_detail',payment_detail_header)" class="btn btn-link btn-link-custom">
...
export default {
  data() {
    return {
      ...
      form: new Form({
        ...
        end_date:null,
        out_date:null,
        page_title:'slr_payment_details_list',
        downloadType:1
      }),
    };
  },
  methods: {
    ...
    getAllPaymentDetails() {
      axios.post(this.BASE_URL + "api/slr_get_payment_detail_list", this.form)
        .then(({ data }) => {
    ...
    openSlipModal(){
        axios.post(this.BASE_URL + "api/slr_unpaid_payment_list", this.form)
        .then(({ data }) => {
    ...
    unpaid_data_download(download_flag=1){
        axios.post(this.BASE_URL + "api/slr_payment_unpaid_data_download", this.form)
        .then(({ data }) => {
            this.downloadFromUrl(data);
        })
        // .catch(() => {
