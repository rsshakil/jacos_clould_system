          ...
          <tr>
            ...
            <td>
                <select class="form-control" v-model="form.mes_lis_inv_lin_det_pay_code">
                    ...
                    <option v-for="(opt, i) in mes_lis_inv_lin_det_pay_code_list" :key="i" :value="i">
                        {{ opt }}
                    </option>
                </select>
            </td>
        ...
        >
          ...
          <tbody>
            <tr v-for="(value,index) in paymentdetailTopTable.data" :key="index">
              <td>{{ paymentdetailTopTable.current_page * form.per_page - form.per_page + index + 1 }}</td>
  ...
  methods: {
    ...
    showAllCustomerCode(){
     let loaderss = Vue.$loading.show();
      ...
      axios.post(this.BASE_URL + "api/slr_get_payment_trade_code_list", this.form)
        .then(({ data }) => {
    ...
    getAllPaymentDetails(page=1) {
      ...
      axios.post(this.BASE_URL + "api/slr_get_payment_item_detail_list", this.form)
        .then(({ data }) => {
...
searchByFormData() {
  ...
  created() {
      this.form = this.$store.getters['slrPaymentItemDetailsModule/getFormData'];
