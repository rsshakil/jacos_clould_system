...
class CreateDataReceiveVouchersTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('data_receive_vouchers', function (Blueprint $table) {
            ...
            $table->string('mes_lis_acc_log_del_transportation_code', 20)->comment('輸送手段');
            ...
            $table->string('mes_lis_acc_log_maker_code_for_receiving', 20)->default('')->comment('入荷管理用メーカーコード');
            $table->string('mes_lis_acc_log_delivery_slip_number', 20)->default('')->comment('センター納品書番号');
            ...
            $table->date('mes_lis_acc_tra_dat_revised_delivery_date')->nullable()->comment('訂正後直接納品先納品日');
            $table->date('mes_lis_acc_tra_dat_revised_delivery_date_to_receiver')->nullable()->comment('訂正後最終納品先納品日');
            ...
            $table->decimal('mes_lis_acc_tra_tax_tax_rate', 3, 1)->comment('税率');
            ...
            $table->decimal('mes_lis_acc_tot_fre_unit_weight_total', 13, 3)->comment('重量合計');
    ...
    {
        Schema::dropIfExists('data_receive_vouchers');
    }
}
