...
class CreateBmsReceivesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('bms_receives', function (Blueprint $table) {
            $table->increments('bms_receive_id')->unsigned()->comment('bms_receive');
            ...
            $table->string('mes_lis_acc_log_del_route_code', 20)->comment('納品経路');
            $table->string('mes_lis_acc_log_del_delivery_service_code', 20)->comment('便No');
            ...
            $table->string('mes_lis_acc_log_log_receiver_abbr_name', 20)->comment('最終納品先略称（印字用）');
            ...
            $table->string('mes_lis_acc_log_maker_code_for_receiving', 20)->comment('入荷管理用メーカーコード');
            $table->string('mes_lis_acc_log_delivery_slip_number', 20)->comment('センター納品書番号');
            ...
            $table->date('mes_lis_acc_tra_dat_order_date')->comment('発注日');
            $table->date('mes_lis_acc_tra_dat_delivery_date')->comment('直接納品先納品日');
            $table->date('mes_lis_acc_tra_dat_delivery_date_to_receiver')->comment('最終納品先納品日');
            $table->date('mes_lis_acc_tra_dat_revised_delivery_date')->comment('訂正後直接納品先納品日');
            $table->date('mes_lis_acc_tra_dat_revised_delivery_date_to_receiver')->comment('訂正後最終納品先納品日');
            ...
            $table->string('mes_lis_acc_lin_qua_package_indicator',20)->comment('発注荷姿コード');
            $table->string('mes_lis_acc_lin_qua_ord_quantity',20)->comment('発注数量（バラ）');
            $table->string('mes_lis_acc_lin_qua_ord_num_of_order_units',20)->comment('発注数量（発注単位数）');
            $table->string('mes_lis_acc_lin_qua_shi_quantity',20)->comment('出荷数量（バラ）');
            $table->string('mes_lis_acc_lin_qua_shi_num_of_order_units',20)->comment('出荷数量（発注単位数）');
            $table->string('mes_lis_acc_lin_qua_rec_quantity',20)->comment('受領数量（バラ）');
            $table->string('mes_lis_acc_lin_qua_rec_num_of_order_units',20)->comment('受領数量（発注単位数）');
            $table->string('mes_lis_acc_lin_qua_rec_reason_code',20)->comment('訂正区分');
            $table->string('mes_lis_acc_lin_fre_unit_weight',20)->comment('取引単位重量');
            ...
            $table->string('mes_lis_acc_lin_fre_shipment_weight',20)->comment('出荷重量');
